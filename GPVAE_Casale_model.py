...
def forward_pass_Casale(data_batch, vae, a, B, c, V, beta, GP, clipping_qs=False):
    ...
    # log variance term
    log_var = tf.reduce_sum(tf.math.log(qnet_var))
...
def forward_pass_Casale_tabular(data_batch, vae, a, B, c, V, beta, GP, clipping_qs=False):
    """
    Forward pass for Casale's GP-VAE model.
    ...
    # recon term
    recon_images_logits = tf.cast(vae.decode(latent_samples), tf.float64)
    recon_images = recon_images_logits
    recon_loss = tf.reduce_sum((data_Y - recon_images_logits) ** 2)
...
def predict_test_set_Casale(test_images, test_aux_data, train_aux_data, vae, GP, V,
    ...
    if take_mean:
        ...
        var = tf.linalg.diag_part(var)  # note how here variance is the same across all latent channels, contrary to SVGPVAE
    ...
    recon_images_test = tf.cast(vae.decode(latent_samples_test), dtype=tf.float64)
...
class casaleGP:
    ...
    def taylor_coeff(self, Z, V):
        ...
        for l in range(L):
            ...
            c.append(0.5*(- tf.matmul(z_T, tf.matmul(K_inv, tf.matmul(K_inv, z))) + tf.linalg.trace(K_inv)))
...
class casaleGPTabular:

    dtype = np.float64

    def __init__(self, fixed_gp_params, object_vectors_init, object_kernel_normalize, ov_joint, RE_cols, aux_cols, jitter=1e-6):
        """
        GP class for rotated MNIST data, based on Casale's paper.
        :param fixed_gp_params:
        :param object_vectors_init: initial value for object vectors (PCA embeddings).
    ...
    def V_matrix(self, aux_data_train, train_ids_mask):
        ...
        # construct \Tilde{V}
        train_object_ids = tf.sort(tf.unique(aux_data_train[:, n_RE_cols - 1]).y)
        # train_angles_unique = tf.sort(tf.unique(aux_data_train[:, n_RE_cols:loc_object]).y)
        train_angles_unique = tf.sort(tf.unique(aux_data_train[:, 1]).y) # problematic with 2D aux columns like location
        if n_aux_cols == 1:
            train_angles_unique = tf.expand_dims(train_angles_unique, axis=1)
        object_vectors = tf.gather(self.object_vectors, tf.cast(train_object_ids, dtype=tf.int64))
        if self.object_kernel_normalize:
            object_vectors = object_vectors / tf.math.reduce_euclidean_norm(object_vectors, axis=1, keepdims=True)
