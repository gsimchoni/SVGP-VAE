...
from SVGPVAE_model import forward_pass_SVGPVAE_mnist, mnistSVGP, forward_pass_standard_VAE_rotated_mnist, \
...
def run_experiment_rotated_mnist_SVGPVAE(args, args_dict):
    ...
    with graph.as_default():
        ...
        if "SVGPVAE" in args.elbo:  # SVGPVAE
            ...
            C_ma, lagrange_mult, mean_vectors = forward_pass_SVGPVAE_mnist(input_batch,
                                                                     beta=beta,
                                                                     vae=VAE,
                                                                     svgp=SVGP_,
                    ...
                    while True:
                        try:
                            ...
                            if MSE_eval < best_eval_loss:
                                ...
                                best_loss_counter += 1
                                if best_loss_counter == args.patience:
                                    stop_training = True
                            break
                if stop_training:
                    break
...
if __name__=="__main__":
    ...
    parser_mnist.add_argument('--patience', type=int, default=3, help="Eval recon_loss no. of epochs to wait before early stopping")
